...
Page({
    ...
    toFeedback(e) {
        ...
        wx.navigateTo({
            url: `/pages/feedback/feedback?id=${item.id}&goodsId=${item.goodsId}&item=${JSON.stringify(item)}`
        })
    },
    // 保存选择的申请原因
    setChoose(e) {
            ...
            wx.requestPayment({
                ...
                success: result => {
                    ...
                    util.requester.get("/frontapi/store/order/confirm", {
                        ...
                        setTimeout(() => {
                            wx.redirectTo({
                                url: '/pages/pay-success/pay-success?id=' + res.datas.orderId + '&type=1',
                            })
                        }, 1200)
                    })
                },
    ...
    toRefundDetail(e) {
        wx.navigateTo({
            url: '/pages/refund-detail/refund-detail?id=' + e.currentTarget.dataset.id,
        })
    },
    /**
     * 获取formId
